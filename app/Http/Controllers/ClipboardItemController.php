...
class ClipboardItemController extends Controller
{
    ...
    public function store(Request $request, string $code): JsonResponse
    {
        ...
        broadcast(new ClipboardEvent($room->code, 'add', [
            'item' => $item->toBroadcast(),
        ]))->toOthers();
    ...
    public function destroy(string $code, string $uuid): JsonResponse
    {
        ...
        $deleted = $room->items()->where('uuid', $uuid)->delete();
        $room->touchActivity();

        if ($deleted > 0) {
            broadcast(new ClipboardEvent($room->code, 'delete', [
                'id' => $uuid,
            ]))->toOthers();
        }
    ...
    public function clear(string $code): JsonResponse
    {
        ...
        broadcast(new ClipboardEvent($room->code, 'clear'))->toOthers();
